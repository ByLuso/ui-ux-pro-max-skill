...
@router.get("/meta")
def meta(region: Region = Depends(region_param)) -> dict:
    """Bounds geográficos, degradado de color y pesos por defecto para los sliders."""
    try:
        return scoring.get_meta(region)
...
def heatmap_png(
    ...
    try:
        png_bytes = scoring.render_heatmap_png(region, weights)
    except httpx.HTTPError as error:
...
def breakdown(
    ...
    weights = _weights(w_lithology, w_slope, w_vegetation, w_water, w_known_sites)
    try:
        result = scoring.get_breakdown(region, lon, lat, weights)
    except httpx.HTTPError as error:
